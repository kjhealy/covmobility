---
title: "Get Started with covmobility"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{covmobility}
...
The datasets are large (especially `google_mobility`), so lazy loading of the tibbles is disabled by default. This means you will have to explicitly load them with `data()` once per session, as needed. For example, to load the small country codes table:
...
The same applies to `apple_mobility` and `google_mobility`. 
